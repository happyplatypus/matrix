...
### datagen work
d<-fread("/home/puru/Dropbox/data/results-zippers.data")
head(d)
d1<-cumsum(d[,-1,with=FALSE])
d1$time<-1:nrow(d1)
d3<-melt(d1,id.vars = 'time')
d3
p <- plot_ly(d3, x=~time,y = ~value, color = ~variable,type="area")
p
